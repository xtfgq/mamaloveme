...
//	private ImageView isLine;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.act_doctordetails);
	...
	public void goChat(View v) {
		if (!hasInternetConnected()) {
			Toast.makeText(DoctorDetailsActivity.this, "网络数据获取失败，请检查网络设置", 1).show();
			return;
		}
	...
	private void AskOrderInquiry() {
		...
		JsonAsyncTask_Info task = new JsonAsyncTask_Info(DoctorDetailsActivity.this, true,
				doProcess);
			...
			public void processJsonObject(Object result) {
				...
				try {
					...
					if (AskByOne == null || "".equals(AskByOne)
						...
						Intent i = new Intent(DoctorDetailsActivity.this, PayActivity.class);
						i.putExtra("AskOrderID", AskOrderID);
						i.putExtra("DoctorId", DoctorID);
						i.putExtra("HospitalID", HospitalID);
						i.putExtra("DoctorName", DoctorName);
...
//		isLine = (ImageView) findViewById(R.id.iv_isonline);
	...
	protected void onResume() {
		...
		user = LocalAccessor.getInstance(DoctorDetailsActivity.this).getUser();
		if (!hasInternetConnected()) {
			Toast.makeText(DoctorDetailsActivity.this, "网络数据获取失败，请检查网络设置", 1).show();
			return;
		}
			...
			public void processJsonObject(Object result) {
				...
				try {
					...
					if ("0".equals(array.getJSONObject(0).getString(
							"MessageCode"))) {
						Toast.makeText(DoctorDetailsActivity.this, "取消关注成功！", 1).show();
						btnguanzhu.setText("关注");
		...
		JsonAsyncTaskOnComplete doProcess = new JsonAsyncTaskOnComplete() {
			...
			public void processJsonObject(Object result) {
				...
				try {
					...
					if ("0".equals(array.getJSONObject(0).getString(
							"MessageCode"))) {
						Toast.makeText(DoctorDetailsActivity.this, "关注成功！", 1).show();
						btnguanzhu.setText("取消关注");
					} else if ("1".equals(array.getJSONObject(0).getString(
							"MessageCode"))) {
						Toast.makeText(
