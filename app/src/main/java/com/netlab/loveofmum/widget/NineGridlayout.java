...
public class NineGridlayout extends ViewGroup {
    ...
    public NineGridlayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext=context;
        totalWidth=ScreenUtils.getScreenWidth(context)-ScreenUtils.dip2px(context,80);
    }
    ...
    private void layoutChildrenView(){
        ...
        for (int i = 0; i < childrenCount; i++) {
           ...
           final int postition= ((Image) listData.get(i)).getPosition();
            childrenView.setOnClickListener(new OnClickListener() {
				
				@Override
				public void onClick(View v) {
					// TODO Auto-generated method stub
					Intent intent=new Intent(mContext,ImagePagesAct.class);
    ...
    public void setImagesData(List<Image> lists) {
        ...
        for(int m=0;m<lists.size();m++){
        	imglist.add(lists.get(m).getUrl());
        }
        //这里做一个重用view的处理
        if (listData == null) {
            int i = 0;
