...
import com.netlab.loveofmum.widget.XScrollView.ConvertUtils;
...
//    private Paint mTest = new Paint();
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        mScaleMarkHeight = mBorderRectF.height() * 0.5f - mTextHeight;
        refreshCanvas();
    }
    ...
    private void drawMarkName(Canvas canvas, boolean isBigMark, float offsetX, int markValue, int offsetBigMark) {
        ...
        if (offsetX != 0) {
            ...
            canvas.drawText(text, centerX + offsetX - mTextRect.centerX(), centerY, mScaleMarkPaint);
        }
    ...
    private void drawOneMark(Canvas canvas, boolean isBigMark, float offsetX) {
        ...
        if (isBigMark) {
            ...
            scaleMarkHeight = mScaleMarkHeight * 0.5f;
        }
    ...
    private void init() {
        ...
        mPerMM2PX = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_MM, 1, dm);
        mTextHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, dm);
    ...
    private void initPaints() {
        ...
        mScaleMarkPaint.setColor(0xffffdfbe);
...
//        mTest.setTextSize(mTextHeight);
        ...
        mPadding = ConvertUtils.dp2Px(getResources(), 8);
    }
...
//                mBorderRectF.centerY(), mTest);
    ...
    private void drawMarkPoint(Canvas canvas) {
        ...
        markPoint.setBounds(centerX - markPoint.getIntrinsicWidth() / 2, 0, centerX + markPoint.getIntrinsicWidth() / 2, canvas.getHeight());
